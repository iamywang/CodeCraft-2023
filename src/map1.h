...
// 配置机器人到每个工作台的距离
void setRobotPlatformDistanceDirectionTime1(int robot_id) {
    // 获取机器人坐标
...
void greedyAlg1(int frame_id, int money) {
    cout << frame_id << endl;
    cout << flush;
    ...
    for (int robot_idx = 0; robot_idx <= 3; robot_idx++) {
        ...
        if (robot_item == 0) {
            ...
            // 需要卖出物品
            vector<pair<double, int>> distance_id_0_0;
            vector<pair<double, int>> distance_id_0_1;
            vector<pair<double, int>> distance_id_0_2;
            for (int i = 0; i < item_demand[robot_item].size(); i++) {
                double raw_distance = robots[robot_idx]->platform_distance[item_demand[robot_item][i]];
                int material_distance = 0;
                int still_need = item_recipes.find(platforms[item_demand[robot_item][i]]->id)->second - platforms[item_demand[robot_item][i]]->material_state;
                for (int j = 1; j <= 7; j++) {
                    if (1 << j & still_need) {
                        material_distance++;
                    }
                }
                if (material_distance == 1)
                    distance_id_0_0.push_back(make_pair(raw_distance, item_demand[robot_item][i]));
                else if (material_distance == 2)
                    distance_id_0_1.push_back(make_pair(raw_distance, item_demand[robot_item][i]));
                else
                    distance_id_0_2.push_back(make_pair(raw_distance, item_demand[robot_item][i]));
            }
            if (distance_id_0_0.size() > 0)
                robots[robot_idx]->platform_distance_sort_sell = sortDistance(distance_id_0_0);
            else if (distance_id_0_1.size() > 0)
                robots[robot_idx]->platform_distance_sort_sell = sortDistance(distance_id_0_1);
            else if (distance_id_0_2.size() > 0)
                robots[robot_idx]->platform_distance_sort_sell = sortDistance(distance_id_0_2);
